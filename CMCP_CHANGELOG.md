...
## 2026-09-20 — Canon046 identity hardening and RC revalidation

- Re-established a clean `initial-platform-primitive` baseline at `5d875e1945462b21c48963621b4d3350f09ebe5e`, tracking `origin/initial-platform-primitive` 0/0 before this pass.
- Re-read Tabling package/docs/source topology, mandatory Objecting/Cruding/Viewing/Interfacing contour, Gating companion material, and authoritative Canonization texts including Canon001, Canon008, Canon018, Canon019, Canon021, Canon023, Canon024, Canon026, and Canon046.
- Target-to-canon mapping: `tabling/table` => `App\\Tabling\\` / `Table*`; role-first source roots remain canonical; no Domain/Application/Infrastructure or Port/Adapter/Adaptor topology; no generic CRUD execution/routes; Collectioning remains the only direct first-party runtime dependency because reverse Cruding/Viewing/Interfacing edges would create dependency cycles and Objecting has no Tabling-owned runtime import.
- Market check against current TanStack Table and AG Grid documentation confirms mature expectations around controlled table state, server-side filtering/sorting/pagination, selection, grouping/aggregation, and large-data server-side operation. Tabling already covers the provider-neutral/backend portions; client rendering, virtualization engines, CRUD execution, and persistence remain outside its responsibility.
- RC-critical finding: one active PHPUnit fixture still used `tenantId` as a composite identifier token. Canon046 now requires canonical platform identity vocabulary `vendorId` / `vendor_id` backed by `VendorEntity.id`. Replaced that fixture token with `vendorId`; test semantics are unchanged because the case only verifies composite-identifier rejection.
- Growth workstream remains separate: tree/hierarchical row metadata, pivot configuration, richer provider capability negotiation, and additional provider adapters are post-RC unless a concrete consumer contract makes them necessary.
- Gates: rerun PHPUnit, PHPStan, PHP-CS-Fixer, production Composer validation, root Composer validation/audit, inspect final diff/status, then integrate only the owned change set.
- Verification: `composer quality` is green with PHPUnit 32/32 and 193 assertions, branch-coverage execution, PHPStan level 8, PHP-CS-Fixer dry-run, and production-manifest validation. Root Composer strict/check-lock validation is green and Composer audit reports no security advisories.
